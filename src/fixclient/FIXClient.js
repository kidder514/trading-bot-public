...
const { fixStringToDescriptive, SYMBOL_TO_PAIR_MAP, now } = require('../util/util');
...
class FIXClient {
  ...
  sendLogon() {
    ...
    console.log(`[${now()}] Logon ===========================================`);
    // console.log(fixStringToDescriptive(messages[0].string));
    // console.log(messages[0].string);
  ...
  sendNewOrder({ label, securityObj, orderQty, direction, posMaintRptID }) {
    ...
    const message = order.data.join('\x01');
    console.log(`[${now()}] Order: Sending ===========================================`);
    // console.log(fixStringToDescriptive(message));
    // console.log(message);
